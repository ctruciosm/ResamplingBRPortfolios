...
#################################
setwd("/Users/ctruciosm/Downloads/Resampling")
rm(list = ls())
library(mvtnorm)
library(POET)
library(Matrix)
...
#            Data              #  
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
monthly_data <- read_xlsx("economatica_ibrx_mensal.xlsx", na = c("-", "NA"), skip = 3, col_types = c("text", rep("numeric", 97)))
...
monthly_data <- monthly_data %>%
  ...
  dplyr::filter(Data >= '2000-01-01', Data < '2023-01-01')

ibovespa <- monthly_data %>% dplyr::select(IBOV)
monthly_data <- monthly_data %>% dplyr::select(-Data, -IBOV)
...
for (i in 1:OoS) { 
  ...
  set.seed(i + 2468)
  w_mv_aux <- calculate_portfolio_weights(x = r_ins, type = "mv",  nboot, factors = ibov)
  w_ef_aux <- calculate_portfolio_weights(x = r_ins, type = "ef",  nboot, factors = ibov, lambda = lambda)
...
oos_results <- rbind(apply(Rport, 2, medidas, rf), apply(to, 2, mean), apply(sspw, 2, mean))
 ...
 t(oos_results) %>%
   ...
   save_kable(keep_tex = T, file = paste0("Results/mvp_2_", InS, "_ibov.tex"))
...
t(oos_results) %>% 
  ...
  save_kable(keep_tex = T, file = paste0("Results/ef_2_", InS, "_ibov.tex"))
