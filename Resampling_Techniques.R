...
michaud_parametric_bootstrap <- function(x, B = 500, type = "tp", riskfree = 0.005, lambda = 2) {
  ...
  for (j in 1:B) {
    ...
    Sigma_boot_hat <- cov_estim(c_returns_boot)
...
factor_parametric_bootstrap <- function(x, B = 500, n_factors = 1, type = "tp", riskfree = 0.005, lambda = 2, factors = NULL) {
  ...
  Sigma_F <- ifelse(n_factors == 1, var(factors[1:(nobs - 1), ]), cov_estim(matrix(factors[1:(nobs - 1), ], ncol = n_factors)))
  w_boot <- matrix(NA, ncol = p, nrow = B)
  ...
  for (j in 1:B) {
    ...
    Sigma_e_boot_hat <- (1/model_boot$df.residual) * (nobs - 2) * cov_estim(epsilon_hat_boot)
