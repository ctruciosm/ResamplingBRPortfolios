...
factor_parametric_bootstrap <- function(x, B = 500, n_factors = 1, option_list = list(type = 'minvol'), factors = NULL) {
  nobs <- nrow(x)
  p <- ncol(x)
  if (is.null(factors)) {
  ...
  Sigma_F <- cov(matrix(factors[-nobs, ], ncol = n_factors))
  
  w_boot <- matrix(NA, ncol = p, nrow = B)
  for (j in 1:B) {
    epsilon_boot <- rmvnorm(nobs - 1, rep(0,p), Sigma_e_hat)
    returns_boot <- matrix(rep(alpha_hat, nobs - 1), ncol = p, byrow = TRUE) + factors[-nobs, ] %*% beta_hat + epsilon_boot
    model_boot <- lm(as.matrix(returns_boot)~ factors[-nobs, ])
    alpha_hat_boot <- coef(model_boot)[1, ]
    beta_hat_boot <- matrix(coef(model_boot)[-1, ], ncol = p)
    epsilon_hat_boot <-  model_boot$residuals
    mu_boot_hat <- alpha_hat_boot + apply(matrix(factors[-nobs, ], ncol = n_factors),2,mean) %*% beta_hat_boot 
...
factor_bootstrap <- function(x, B = 500, n_factors = 1, option_list = list(type = 'minvol'), factors = NULL) {
  nobs <- nrow(x)
  p <- ncol(x)
  if (is.null(factors)) {
    x_c <- scale(x, center = TRUE, scale = FALSE)
