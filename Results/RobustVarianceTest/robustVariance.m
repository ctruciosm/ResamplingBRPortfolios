function [diff, delta, sigi, pvalpw, pvalboot] = robustVariance(data,boot,M,bl,kernel,extsim,bootMat)
...
z_T = abs(delta - H0)./(sqrt(varde/t));     %studentization of data
...
if boot==1
    ...
    [bsstat] = bsstats11(X,Xbootind,delta,bl,L,r,t);
    
    pvalboot=(sum(bsstat>=abs(z_T))+1)/(M+1);
end
