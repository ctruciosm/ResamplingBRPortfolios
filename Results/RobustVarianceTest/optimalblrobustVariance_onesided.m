function [optbl blcand] = optimalblrobustVariance_onesided(data,silent,M,MBM,kernel,blcand,avbl,alph,Tstart,extsimSB,extSB,extsimMB,extMB)
...
for m=1:M       %%% first level bootstrap
    ...
    for b=1:length(blcand)
        ...
        [bsstatistics] = bsstats11_onesided(X,Xbootind,mubi,blc,floor(T/blc),mod(T,blc),T);
        
        [~,rej] = critvalues1_onesided(bsstatistics,mubi,sigbl,theta_0star,alph);
